...
public abstract class RelationalQueryRepositoryBase<T> : IRelationalQueryRepository<T> where T : class
{
	protected readonly IQueryable<T> Query;

	protected RelationalQueryRepositoryBase(DbContext context) : this(context.Set<T>())
	{
	}

	protected RelationalQueryRepositoryBase(IQueryable<T> query)
	{
		Query = query;
	}
...
public abstract class RelationalQueryRepositoryBase<TParent, TChild> : RelationalQueryRepositoryBase<TChild>, IRelationalQueryRepository<TParent, TChild> where TParent : class where TChild : class, TParent
{
	protected RelationalQueryRepositoryBase(DbContext context) : base(context.Set<TParent>().OfType<TChild>())
	{
	}
}
